...
#
rpart <-
    function(formula, data, treat, weights, subset, na.action = na.rpart, method,
             model = FALSE, x = FALSE, y = TRUE, parms, control, cost, ...)
      # add treat as treatment status vector
      # we use parms = min_node_size
      # lots of arguments to be deleted
      # also we need to change the minbucket in control
      # method is used for cross-validation now.
      
{
    ...
    nobs <- nrow(X)
    nvar <- ncol(X)
    # add the check of arguments for treat:
    if (missing(treat)) {
      stop ("You need to include a vector for treatment status.")
      } else if (length(treat) != nobs) {
        stop ("The length of treatment status is not matched to number of observations.")
	    } else {
	      condition = levels(as.factor(treat)) == c("0","1")
	      if(!condition[1] || !condition[2])
	      stop ("The treatment status vector should only contain elements 0, 1.")
	    }
    
    # change the method of causal tree, here we only use "anova"
	  ##################################
    if (missing(method)) {
      ...
      ##   standalone debugging.
        ...
        if (missing(parms)) 
          get(paste("rpart", method, sep = "."),
              envir = environment())(Y, treat, offset, , wt)
        else
          get(paste("rpart", method, sep = "."),
              envir = environment())(Y, treat, offset, parms, wt)
	    }
