...
xpred(SEXP ncat2, SEXP method2, SEXP opt2,
      ...
      SEXP all2, SEXP cp2, SEXP toprisk2, 
      SEXP nresp2)
{
    char *errmsg;
    int i, j, k, n;
    ...
    int maxcat, ncp;
    int xgroup;
    double temp, temp2, total_wt, old_wt, total_treat, old_treat;
    int *savesort;
    double *dptr;               /* temp */
    int nresp;
    double toprisk;
    ...
    (*rp_init) (n, rp.ydata, treat, maxcat, &errmsg, parms, &rp.num_resp, 1, wt);
    ...
    for (i = 0; i < rp.n; i++){
      total_wt += rp.wt[i];
      total_treat += rp.treat[i];
    }
    old_wt = total_wt;
    old_treat = total_treat;
	 ...
	 */
	xtree->num_obs = k;
	(*rp_init) (k, rp.ytemp, rp.treatemp, maxcat, &errmsg, parms, &ii, 2, rp.wtemp);
