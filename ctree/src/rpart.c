...
rpart(SEXP ncat2, SEXP method2, SEXP opt2,
      ...
      SEXP ymat2, SEXP treat2, SEXP xmat2, 
      SEXP wt2, SEXP ny2, SEXP cost2)
{
    ...
    int xvals;
    double *wt, *parms;
    double *treat;  /* treatment status vector */
    ...
    if (asInteger(method2) <= NUM_METHODS) {
      ...
      // rp_cross_eval = func_table[i].cross_eval;
      // where we can choose the out-of-sample measure for cross-validation 
      /* need to insert #####################>>>>>>>>>>>>>>>>#########...........*/
    ...
    /* max primary splits = max competitors + 1 */
    if (rp.maxpri < 1)
      rp.maxpri = 1;
    ...
    rp.n = nrows(xmat2);
    n = rp.n;                   
    /* I get tired of typing "rp.n" 100 times
    * below */
    rp.nvar = ncols(xmat2);
    ...
    for (i = 0; i < n; i++) {
	    rp.ydata[i] = dptr;
      /* notice: in this version we only deal with y as a one-colum vector */
      for (j = 0; j <  rp.num_y; j++){
        if (fabs(rp.ydata[i][j]) > temp2) temp2 = fabs(rp.ydata[i][j]);        
      }
	    dptr += rp.num_y;
    }
    /* find the maximum absolute value of y values */
    rp.max_y = temp2;
    ...
    rp.treatemp = (double *) ALLOC(n, sizeof(double)); 
    ...
    if (maxcat > 0) {
      ...
      rp.ltreat = (double *) ALLOC(2 * maxcat, sizeof(double));
      rp.rtreat = rp.ltreat + maxcat;
    } else
    ...
    for (i = 0; i < n; i++) {
	    rp.which[i] = 1;
	    temp += wt[i];
      temp2 += treat[i]; // total treated cases
    }
    
    i = (*rp_init) (n, rp.ydata, treat, maxcat, &errmsg, parms, &rp.num_resp, 1, wt);
    if (i > 0) error(errmsg);
    ...
    tree->num_obs = n;
    tree->sum_wt = temp;
    tree->sum_treat = temp2;

    (*rp_eval) (n, rp.ydata, treat, tree->response_est, &(tree->risk), wt, rp.max_y);
